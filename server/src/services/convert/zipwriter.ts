...
const CRC_TABLE = (() => {
  ...
  // Stryker disable next-line EqualityOperator: `<= 256` writes t[256], an
  // out-of-bounds index a Uint32Array silently drops; reads never go past 255.
  for (let n = 0; n < 256; n++) {
    let c = n;
...
export function dosDateTime(d = new Date()): { time: number; date: number } {
  const time =
...
export class ZipWriter {
  ...
  add(name: string, content: Buffer | string, { store = false }: { store?: boolean } = {}): this {
    // Stryker disable next-line StringLiteral: '' as an encoding falls back to
    // utf8, so blanking it is unobservable.
    const data = Buffer.isBuffer(content) ? content : Buffer.from(content, 'utf8');
    const crc = crc32(data);
    // Stryker disable next-line ObjectLiteral: the deflate level trades size for
    // speed only; every level round-trips identically.
    const compressed = store ? data : zlib.deflateRawSync(data, { level: 9 });
    this.entries.push({
      // Stryker disable next-line StringLiteral: '' encoding falls back to utf8.
  ...
  toBuffer(): Buffer {
    ...
    // `Buffer.alloc` zero-fills, so the many reserved/zero header fields (flags,
    // extra-length, disk numbers, internal/external attributes, comment lengths)
    // are already 0 and need no explicit write.
