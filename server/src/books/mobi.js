...
export function mobiMeta(buf) {
  const meta = { title: '', authors: [], genres: [], lang: '', docdate: '', series: null };
  try {
    if (buf.length < 78 || buf.toString('latin1', 60, 64) !== 'BOOK') return meta;
    ...
    if (rec0.toString('latin1', 16, 20) !== 'MOBI') return meta;
    ...
    if (exthFlags & 0x40) {
      ...
      if (rec0.toString('latin1', start, start + 4) === 'EXTH') {
        ...
        let p = start + 12;
        const seriesName = { n: null, idx: null };
...
export function mobiCover(buf) {
  try {
    ...
    const base =
      firstImageIndex > 0 && firstImageIndex < numRecords ? firstImageIndex : null;

    const tryRecord = (recNo) => {
...
function firstImage(buf, offsets, start) {
  for (let i = start; i < offsets.length - 1; i++) {
    const r = buf.subarray(offsets[i], offsets[i + 1]);
    if (r.toString('latin1', 0, 4) === 'FLIS' || r.toString('latin1', 0, 4) === 'FCIS')
      break;
    const mime = sniffMime(r);
