...
function decodeBackwardVarint(record, end) {
  let value = 0;
  let bytesConsumed = 0;
  for (let k = 1; k <= 4; k++) {
    const byte = record[end - k];
...
function parseExth(rec0, start) {
  const meta = { authors: [], title: null, language: null, coverOffset: null };
...
export function mobiToIr(buf) {
  ...
  htmlAll = htmlAll.replace(/<img\b[^>]*recindex=["']?(\d+)["']?[^>]*>/gi, (m, n) => {
  ...
  ir.chapters = (rawChapters.length ? rawChapters : [stripMobiWrappers(htmlAll)]).map((h, i) => {
...
function stripMobiWrappers(html) {
  return String(html || '')
    .replace(/<\?xml[^>]*\?>/gi, '')
...
function assemblePalmDb(title, records) {
