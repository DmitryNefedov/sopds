...
for (const from of CONVERTIBLE) {
  for (const to of CONVERTIBLE) {
    ...
    test(`${from} -> ${to} produces a parseable ${to} with text preserved`, () => {
      const src = fs.readFileSync(samples[from]);
      ...
      const srcWords = srcText.split(' ').filter((w) => w.length > 4).slice(0, 40);
      const hit = srcWords.filter((w) => outText.includes(w)).length;
      assert.ok(
