...
function fb2({ encoding = 'utf-8', cover = true, coverpage = true, img = PNG, titleBytes = null }) {
  ...
  return Buffer.concat([Buffer.from(head, 'latin1'), titleBytes, Buffer.from(tail, 'latin1')]);
}
...
test('fb2: tolerates base64 with embedded whitespace / newlines', () => {
  const messy = fb2({}).toString('utf8').replace(
    ...
    (m, body) => m.replace(body, body.replace(/(.{10})/g, '$1  \n')),
  );
